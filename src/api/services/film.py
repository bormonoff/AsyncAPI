...
class FilmService:
    ...
    async def get_by_id(self, film_id: str) -> filmmodel.Film | None:
        """Optionally return a film from ES."""
        film = await self.cache_service.get_entity_from_cache("film", film_id)
        if not film:
            film = await self.storage.get_film_by_id(film_id)
            if film:
    ...
    async def get_films_sorted_by_field(
        ...
        films = await self.cache_service.get_list_from_cache(
            "base_film", field_to_filter, field_to_sort, page_size, page_number
        )
        if not films:
            ...
            if not films:
                return None
            await self.cache_service.put_list_to_cache(
                "base_film", field_to_filter, field_to_sort, page_size, page_number, films
            )
        return films

    async def get_films_with_pattern(self, pattern: str, page_size: int, page_number: int) -> list[filmmodel.FilmBase]:
    ...
    def _handle_movie(self, movie: dict[str, Any]) -> filmmodel.FilmBase:
        return filmmodel.FilmBase(id=movie["id"], title=movie["title"], imdb_rating=movie["imdb_rating"])
...
def get_film_service(
    ...
    return FilmService(redis, ElasticMoviesStorage())
